...
<div class="card m-3">
    ...
    <div class="card-body">
        <div class="accordion" id="accordion">
            @if ($medico->atendimentos->isEmpty())
                <p>Esse médico ainda não atendeu nenhum paciente, <a href="{{ route('atendimento.create') }}">clique aqui para agendar um atendimento</a>.</p>
            @endif
            @foreach ($medico->atendimentos->sortByDesc('created_at') as $key => $atendimento) {{-- ->sortBy('nome')--}}
                <div class="accordion-item m-3">
                    <div class="d-flex justify-content-between" id="heading-1">
                        <button class="accordion-button " type="button" data-bs-toggle="collapse" data-bs-target="#accordion-collapse-{{ $atendimento->id }}" aria-expanded="true">
                            ({{ $atendimento->id }}) {{ $atendimento->Paciente->nome }}
                        </button>
                    </div>
