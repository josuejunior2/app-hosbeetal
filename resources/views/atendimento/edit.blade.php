...
<div class="card m-3">
    ...
    @endif
    <div class="card-body">
    <form method="POST" action="{{ route('atendimento.update', ['atendimento' => $atendimento]) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row g-3 mb-4">
            <div class="col-md">
                <div class="mb-3">
                    ...
                    <div class="col">
                        <input type="text" name="data_atendimento" id="data_atendimento" class="form-control" value="{{ old('data_atendimento', \Carbon\Carbon::parse($atendimento->data_atendimento)->format('d/m/Y')) }}" autocomplete="off" readonly/> {{-- value="{{ old('data_nascimento', '') }}" --}}
                ...
                <div class="col">
                    <select class="form-select" type="text" name="medico_id" id="medico_id">
                        ...
                        @foreach($medicos as $m)
                            <option value="{{ $m->id }}" {{ old('medico_id') == $m->id || $atendimento->medico_id == $m->id ? 'selected' : '' }}>
                                {{ $m->nome }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col">
                    <select class="form-select" type="text" name="paciente_id" id="paciente_id">
                        ...
                        @foreach($pacientes as $p)
                            <option value="{{ $p->id }}" {{ old('paciente_id') == $p->id || $atendimento->paciente_id == $p->id ? 'selected' : '' }}>
                                {{ $p->nome }}
                            </option>
                        @endforeach
                    </select>
